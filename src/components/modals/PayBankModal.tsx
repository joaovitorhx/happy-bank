...
export function PayBankModal({ isOpen, onClose, currentPlayer }: PayBankModalProps) {
  const payBank = useGameStore((state) => state.payBank);
  ...
  const handleConfirm = async () => {
    ...
    setLoading(true);
    try {
      await payBank(parseInt(amount), note || undefined);
      playSendSound();
      toast.success('Pago ao banco!');
      setTimeout(handleClose, 500);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Pagamento falhou');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <BottomSheet isOpen={isOpen} onClose={handleClose} title="Pagar ao Banco">
      <AnimatePresence mode="wait">
        {step === 'amount' && (
          ...
          >
            {/* Bank Info */}
            <div className="flex items-center gap-3 p-3 rounded-xl bg-bank/10 border-2 border-bank">
              ...
              <div className="flex-1">
                <p className="text-sm text-muted-foreground">Pagando para</p>
            ...
            <div className="flex justify-between text-sm font-display">
              ...
              <span className={numAmount > currentPlayer.balance ? 'text-destructive' : 'text-primary'}>
        ...
        {step === 'confirm' && (
          ...
          >
            <div className="py-6">
              <p className="text-muted-foreground font-display mb-2">Você está pagando</p>
              <p className="balance-display text-accent mb-4">
                {formatCurrency(numAmount)}
              </p>
              <p className="text-muted-foreground font-display">para o</p>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <GameButton variant="accent" onClick={handleConfirm} disabled={loading}>
